...
// Truncate returns a sub *Float32Seq with specified interval
func (s *Float32Seq) Truncate(interval Interval) *Float32Seq {
	sslice := s.getSlice()
	slice := Truncate(sslice, interval).(Float32s)
	return newFloat32Seq(slice)
}
