...
// Truncate returns a sub *Float32Seq with specified range
func (s *Float32Seq) Truncate(rg Range) *Float32Seq {
	sslice := s.getSlice()
	slice := Truncate(sslice, rg).(Float32s)
	return newFloat32Seq(slice)
}
...
func (s *Float32Seq) Aggregate(fn func(t time.Time, slice Float32s) *float32, duration time.Duration, rg Range) *Float32Seq {
	if fn == nil {
		return s
	}
