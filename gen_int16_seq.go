...
// Truncate returns a sub *Int16Seq with specified range
func (s *Int16Seq) Truncate(rg Range) *Int16Seq {
	sslice := s.getSlice()
	slice := Truncate(sslice, rg).(Int16s)
	return newInt16Seq(slice)
}
...
func (s *Int16Seq) Aggregate(fn func(t time.Time, slice Int16s) *int16, duration time.Duration, rg Range) *Int16Seq {
	if fn == nil {
		return s
	}
