...
func TestUintSeq_NilSafe(t *testing.T) {
	var seq *UintSeq
	seq.Uints()
	seq.Index(1)
	seq.Time(time.Now())
	...
	seq.Value(1)
	seq.MValue(1)
	seq.Traverse(func(i int, v Uint) (stop bool) {
		return false
	})
	seq.Sum()
	seq.Max()
	...
	seq.Merge(func(t time.Time, v1, v2 *uint) *uint {
		return nil
	}, seq)
...
func TestUintSeq_Truncate(t *testing.T) {
	data := RandomUints(100)
	Sort(data)
...
func TestUintSeq_Slice(t *testing.T) {
	data := RandomUints(100)
	Sort(data)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := newUintSeq(data)
			if got := s.Slice(tt.args.i, tt.args.j).Uints(); !reflect.DeepEqual(got, tt.want) {
