...
// Truncate returns a sub *IntSeq with specified range
func (s *IntSeq) Truncate(rg Range) *IntSeq {
	sslice := s.getSlice()
	slice := Truncate(sslice, rg).(Ints)
	return newIntSeq(slice)
}
...
func (s *IntSeq) Aggregate(fn func(t time.Time, slice Ints) *int, duration time.Duration, rg Range) *IntSeq {
	if fn == nil {
		return s
	}
