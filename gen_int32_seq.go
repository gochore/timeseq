...
// Truncate returns a sub *Int32Seq with specified range
func (s *Int32Seq) Truncate(rg Range) *Int32Seq {
	sslice := s.getSlice()
	slice := Truncate(sslice, rg).(Int32s)
	return newInt32Seq(slice)
}
...
func (s *Int32Seq) Aggregate(fn func(t time.Time, slice Int32s) *int32, duration time.Duration, rg Range) *Int32Seq {
	if fn == nil {
		return s
	}
