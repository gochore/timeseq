...
func RandomInt8s(length int) Int8s {
	now := time.Now()
	ret := make(Int8s, length)
	for i := range ret {
		delta := time.Duration(i) * time.Second
		if rand.Float64() < 0.5 {
			delta = -delta
		}
		value := int8(rand.Int31())
...
func TestInt8Seq_NilSafe(t *testing.T) {
	var seq *Int8Seq
	seq.Int8s()
	seq.Index(1)
	seq.Time(time.Now())
	...
	seq.Value(1)
	seq.MValue(1)
	seq.Traverse(func(i int, v Int8) (stop bool) {
		return false
	})
	seq.Sum()
	seq.Max()
	...
	seq.Slice(0, 0)
	seq.Trim(func(i int, v Int8) bool {
		return false
	})
	seq.Merge(func(t time.Time, v1, v2 *int8) *int8 {
		return nil
	}, seq)
	seq.Aggregate(func(t time.Time, slice Int8s) *int8 {
		return nil
	}, time.Hour, Interval{})
}
...
func TestInt8Seq_Int8s(t *testing.T) {
	data := RandomInt8s(100)
	Sort(data)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := NewInt8Seq(data)
			if got := s.Int8s(); !reflect.DeepEqual(got, tt.want) {
...
func TestInt8Seq_Index(t *testing.T) {
	data := RandomInt8s(100)
	Sort(data)
...
func TestInt8Seq_Time(t *testing.T) {
	now := time.Now()
...
func TestInt8Seq_MTime(t *testing.T) {
	now := time.Now()
	yesterday := now.AddDate(0, 0, -1)
	lastMonth := now.AddDate(0, -1, 0)
	lastYear := now.AddDate(-1, 0, 0)
...
func TestInt8Seq_Value(t *testing.T) {
	data := RandomInt8s(100)
	Sort(data)
...
func TestInt8Seq_MValue(t *testing.T) {
	data := RandomInt8s(100)
	Sort(data)
...
func TestInt8Seq_Traverse(t *testing.T) {
	data := RandomInt8s(100)

	type args struct {
		fn func(i int, v Int8) (stop bool)
	}
	tests := []struct {
		name string
		args args
...
func TestInt8Seq_Sum(t *testing.T) {
	data := RandomInt8s(100)
	Sort(data)
	var sum int8
	for _, v := range data {
...
func TestInt8Seq_Max(t *testing.T) {
	data := RandomInt8s(100)
	max := data[0]
	for _, v := range data {
...
func TestInt8Seq_Min(t *testing.T) {
	data := RandomInt8s(100)
	min := data[0]
	for _, v := range data {
...
func TestInt8Seq_First(t *testing.T) {
	data := RandomInt8s(100)
	Sort(data)
...
func TestInt8Seq_Last(t *testing.T) {
	data := RandomInt8s(100)
	Sort(data)
	tests := []struct {
		name string
...
func TestInt8Seq_Percentile(t *testing.T) {
	data := RandomInt8s(100)
	Sort(data)
	for i := range data {
		data[i].Value = int8(i)
...
func TestInt8Seq_Truncate(t *testing.T) {
	data := RandomInt8s(100)
	Sort(data)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := NewInt8Seq(data)
			if got := s.Truncate(tt.args.interval).Int8s(); !reflect.DeepEqual(got, tt.want) {
...
func TestInt8Seq_Slice(t *testing.T) {
	data := RandomInt8s(100)
	Sort(data)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := newInt8Seq(data)
			if got := s.Slice(tt.args.i, tt.args.j).Int8s(); !reflect.DeepEqual(got, tt.want) {
...
func TestInt8Seq_Trim(t *testing.T) {
	data := RandomInt8s(10)
	Sort(data)

	type args struct {
		fn func(i int, v Int8) bool
	}
	tests := []struct {
		name string
		args args
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := NewInt8Seq(data)
			if got := s.Trim(tt.args.fn).Int8s(); !reflect.DeepEqual(got, tt.want) {
...
func TestInt8Seq_Merge(t *testing.T) {
	data := RandomInt8s(10)
	Sort(data)

	type args struct {
		fn    func(t time.Time, v1, v2 *int8) *int8
		slice Int8s
	}
	tests := []struct {
		name string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := NewInt8Seq(tt.data)
			if got := s.Merge(tt.args.fn, WrapInt8Seq(tt.args.slice)).Int8s(); !reflect.DeepEqual(got, tt.want) {
...
func TestInt8Seq_Aggregate(t *testing.T) {
	type args struct {
		fn       func(t time.Time, slice Int8s) *int8
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := NewInt8Seq(tt.slice)
			if got := s.Aggregate(tt.args.fn, tt.args.duration, tt.args.interval).Int8s(); !reflect.DeepEqual(got, tt.want) {
