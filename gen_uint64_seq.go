...
// Truncate returns a sub *Uint64Seq with specified range
func (s *Uint64Seq) Truncate(rg Range) *Uint64Seq {
	sslice := s.getSlice()
	slice := Truncate(sslice, rg).(Uint64s)
	return newUint64Seq(slice)
}
...
func (s *Uint64Seq) Aggregate(fn func(t time.Time, slice Uint64s) *uint64, duration time.Duration, rg Range) *Uint64Seq {
	if fn == nil {
		return s
	}
