...
// Truncate returns a sub *Int64Seq with specified range
func (s *Int64Seq) Truncate(rg Range) *Int64Seq {
	sslice := s.getSlice()
	slice := Truncate(sslice, rg).(Int64s)
	return newInt64Seq(slice)
}
...
func (s *Int64Seq) Slice(i, j int) *Int64Seq {
	...
	sslice := s.getSlice()
	if i < 0 {
		return newInt64Seq(sslice[:j])
	}
	if j < 0 {
		return newInt64Seq(sslice[i:])
	}
	return newInt64Seq(sslice[i:j])
}
...
func (s *Int64Seq) Aggregate(fn func(t time.Time, slice Int64s) *int64, duration time.Duration, rg Range) *Int64Seq {
	if fn == nil {
		return s
	}
