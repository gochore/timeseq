...
func TestUint16Seq_Range(t *testing.T) {
	data := RandomUint16s(100)
	Sort(data)
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				slices: []Uint16s{
					append(Uint16s{data[9]}, data[3:9]...),
				},
			},
			want: data,
		},
...
func TestUint16Seq_Clone(t *testing.T) {
	data := RandomUint16s(10)
	Sort(data)
	tests := []struct {
		name string
